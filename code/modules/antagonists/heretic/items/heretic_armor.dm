...
/obj/item/clothing/suit/hooded/cultrobes/eldritch/cosmic/robes_side_effect(mob/living/user)
	var/obj/item/organ/internal/brain/victim_brain = user.get_organ_slot(ORGAN_SLOT_BRAIN)
	if(!victim_brain)
		return
...
/obj/item/clothing/suit/hooded/cultrobes/eldritch/moon/equipped(mob/user, slot, initial)
	...
	var/obj/item/organ/internal/brain/our_brain = human_user.get_organ_slot(ORGAN_SLOT_BRAIN)
	if(!our_brain)
		return
...
/obj/item/clothing/suit/hooded/cultrobes/eldritch/moon/dropped(mob/living/user)
	...
	var/obj/item/organ/internal/brain/our_brain = wearer.get_organ_slot(ORGAN_SLOT_BRAIN)
	if(our_brain)
...
/atom/movable/screen/moon_health/proc/update_health(obj/item/organ/internal/brain/brain, damage_amount, maximum, required_organ_flag)
	SIGNAL_HANDLER
...
/obj/item/clothing/suit/hooded/cultrobes/eldritch/rust/proc/vomit_your_guts_out(mob/living/carbon/victim)
	if(QDELETED(victim) || !is_equipped(victim))
		return
	victim.vomit(blood = TRUE, stun = FALSE, message = TRUE, harm = TRUE, force = TRUE /* MOB_VOMIT_BLOOD | MOB_VOMIT_MESSAGE | MOB_VOMIT_HARM | MOB_VOMIT_FORCE */)
...
/obj/item/clothing/suit/hooded/cultrobes/void/proc/hide_item(mob/source, obj/item/item, slot)
	SIGNAL_HANDLER
	if(slot & ITEM_SLOT_SUITSTORE)
		item.add_traits(list(TRAIT_NO_STRIP, TRAIT_NO_WORN_ICON, TRAIT_EXAMINE_SKIP), REF(src))
		source.update_suit_storage()

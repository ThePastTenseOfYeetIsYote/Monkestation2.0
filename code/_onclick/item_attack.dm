...
/obj/item/proc/attack(mob/living/target_mob, mob/living/user, list/modifiers, list/attack_modifiers)
	...
	if(final_force > 0)
		log_combat(user, target_mob, "attacked", src.name, "(COMBAT MODE: [uppertext(user.log_istate())]) (DAMTYPE: [uppertext(damtype)])")
...
/mob/living/attacked_by(obj/item/attacking_item, mob/living/user, list/modifiers, list/attack_modifiers)
	...
	if((attacking_item.item_flags & SURGICAL_TOOL) && !(user.istate & ISTATE_HARM) && body_position == LYING_DOWN && (LAZYLEN(surgeries) > 0))
		wounding = CANT_WOUND
